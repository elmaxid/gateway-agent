...
- `node "${CLAUDE_PLUGIN_ROOT}/scripts/gateway-companion.mjs" dispatch [--plan FILE | --task PROMPT:PROFILE ...] [--harness claude|codex|zero|kimi|cline] [--max-concurrency N] [--timeout MS] [--write|--no-write] [--cross-review PROFILE] [--fail-fast] [--dry-run] [--json]`
...
- With `--harness kimi`, every task profile+model must be declared under `[providers.gateway]`/`[models."gateway/<model>"]` in kimi's config.toml; misaligned/undeclared ones are rejected pre-dispatch (exit 2), same timing as zero. `--no-write` is also rejected pre-dispatch with `--harness kimi`.
- With `--harness cline`, every task profile must point at the URL of cline's `litellm` provider; misaligned profiles are rejected pre-dispatch (exit 2), same timing as zero/kimi. Unlike kimi, `--no-write` is allowed (cline has real read-only support).
- With `--harness codex`, dispatch does NOT fall back to claude if codex is missing — it preflight-fails (exit 2), unlike `task --harness codex`, which does fall back to claude for `claude-gateway` profiles. Fail-loud in dispatch applies to codex, zero, kimi, and cline — not just zero.
...
- Profiles must be configured first via `/gateway:setup` (fresh machine: `setup add`, then `setup test`; for zero additionally `setup zero-init` once per machine; for kimi, manually configure the `gateway` provider in `~/.kimi-code/config.toml`; for cline, run `cline auth --provider litellm --baseurl <url> --apikey <key> --modelid <model>` once — see `setup doctor` for a full health check of all five harnesses).
